...
int IsExpression(int i, FILE* ef)
{
	...
	for (; (Data.LexTable[nom].type != LSeparator); nom++)
	{
		...
		if (Data.LexTable[nom].type == LRBraket)
			if (!(IsOperation(Data.LexTable[nom + 1].type)) && (Data.LexTable[nom + 1].type != LSeparator) && (Data.LexTable[nom + 1].type != LRBraket) && (Data.LexTable[nom + 2].type != LDo))
			{
		...
		{
			if (!(IsOperation(Data.LexTable[nom + 1].type)) && (Data.LexTable[nom + 1].type != LRBraket) && (Data.LexTable[nom + 1].type != LSeparator) && (Data.LexTable[nom + 1].type != LDownTo) && (Data.LexTable[nom + 2].type != LDo))
			{
...
int ErrorChecking()
{
	...
	if (Data.LexTable[2].type != LVarType)
	{
		Err_num++;
		fprintf(ef, "%d line %d: \t'Int32_t' expected!\n", Err_num, Data.LexTable[3].line);
	}
	if (Data.LexTable[2].type == LVarType)
	{
		i = 3;
	...
	switch (Balans(i, LEOF, LFor, LDownTo))
	{
	case 1:
		Err_num++;
		fprintf(ef, "line %d: \tToo much 'Downto'!\n", Data.LexTable[j].line);
		break;
	case 2:
		Err_num++;
		fprintf(ef, "line %d: \t'Downto' expected!\n", Data.LexTable[j].line);
		break;
	default:
		break;
	}

	switch (Balans(i, LEOF, LFor, LDo))
	{
	case 1:
		Err_num++;
	...
	for (j = 0;; j++)
	{
		...
		if (Data.LexTable[j].type == LFor)				//перевірка for
		{
			if (Data.LexTable[j + 1].type != LLBraket)
			{
				Err_num++;
			...
			while ((Data.LexTable[j].type != LDownTo) && (Data.LexTable[j].type != LEOF)) j++;
			if (IsExpression(j + 1, ef) != 0)
			{
				Err_num++;
				fprintf(ef, "line %d: \tIdentifier or number expected after 'Downto'!\n", Data.LexTable[j + 1].line);
			}

			while ((Data.LexTable[j].type != LDo) && (Data.LexTable[j].type != LEOF)) j++;
			if (Data.LexTable[j - 1].type != LRBraket)
			{
				Err_num++;
				fprintf(ef, "line %d: \t')' expected before 'Do'!\n", Data.LexTable[j + 1].line);
			}
			if (Data.LexTable[j + 1].type != LStart)
			{
				Err_num++;
				fprintf(ef, "line %d: \t'Startblock' expected!\n", Data.LexTable[j + 6].line);
			}
		}
