...
void CheckPresent()			//Визначення присутності операторів put i get
{
	...
	for (int i = 0; Data.LexTable[i].type != LEOF; ++i)
	{
		...
		if (Data.LexTable[i].type == LGreate || Data.LexTable[i].type == LFor) Data.IsPresentGreate = true;
...
void GenASMCode(const char* str, FILE* f)
{
	...
	for (n = 0; Data.bufExprPostfixForm[n] != 3000; ++n)
	{
		...
		{
			if (Data.LexTable[Data.bufExprPostfixForm[n]].type == LIdentifier)
			{
...
void PrintCode(FILE* f)
{
	...
	for (;; ++i)
	{
		...
		{
			if (!startStack.IsEmpty(&startStack.S))
			{
				int nv = startStack.Pop(&startStack.S);
				int lab = startStack.Pop(&startStack.S);

				fprintf(f, "\tfild %s\n", Data.LexTable[nv - 1].name);
				fputs("\tfld1\n", f);
				fputs("\tfsub\n", f);
				fprintf(f, "\tfistp %s\n", Data.LexTable[nv - 1].name);
		...
		if (l.type == LFor)
		{
			...
			while (Data.LexTable[i].type != LDownTo) i++;

			fprintf(f, "forStart%d:\n", ++lab);
			startStack.Push(lab, &startStack.S);

			int bufi = i;
			i = ConvertToPostfixForm(i);//Генерація постфіксного виразу			
			if (i < 0)
			{
				i = -i;
				puts("IE\n");
			...
			startStack.Push(nv, &startStack.S);
